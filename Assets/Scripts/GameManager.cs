...
public class GameManager : Singleton<GameManager>
{
    ...
    EnemyBlock[,] enemyAllBlocks= new EnemyBlock[BOARDX, BOARDY];
    ...
    void EnemyCheck(int indexX, int blockCount)
    {
        ...
        for (int i = 0; i < blockCount; i++)
        {
            EnemyAllBlocks[indexX, (BOARDY / 2 - 1) - i].BlockEnable(indexX, ((BOARDY / 2 - 1) - i) - blockCount);
        }
    ...
    IEnumerator AutoEnemy()
    {
        while(GameStart)
        {
            if (thisEnmeyBlock == null)
            {
                ...
                int enemyIndexX = UnityEngine.Random.Range(0, BOARDX);
                int enemyIndexY = UnityEngine.Random.Range(BOARDY / 2, BOARDY);
