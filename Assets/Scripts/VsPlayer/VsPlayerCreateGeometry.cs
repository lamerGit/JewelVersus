...
public class VsPlayerCreateGeometry : MonoBehaviour
{
    //다른 플레이어의 블록을 생성하는 클래스

    public GameObject block; //생성할 블록
    ...
    void CreateCircle()
    {
        ...
        for (int i = 0; i < boardx; i++)
        {
            for (int j = 0; j < boardy; j++)
            {
                ...
                VsPlayerBlock tempblock = temp.GetComponent<VsPlayerBlock>();  //컴포넌트받고
                ...
                GameManager.Instance.VsPlayerBlockManager.AllBlocks[i, j] = tempblock; // 게임매니저에 준다.
